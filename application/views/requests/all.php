<h1 class="display-4">Все заявки</h1>
<ul class="nav nav-tabs">
    <li class="nav-item">
        <a class="nav-link active" href="#">Все заявки <span class="badge badge-secondary"><?=$requests_count?></span></a>
    </li>
    <li class="nav-item">
    ...
    <li class="nav-item">
        <a class="nav-link" href="/requests/closed">Завершенные</a>
    </li>
    <li class="nav-item">
    ...
    <?php foreach ($requests as $value): ?>
        <tr>
            ...
            <td class="align-middle text-center" style="width: 30px">
                ...
                <? else: ?>
                    <? if($_SESSION['user']['id_permission'] == 1):?>
                        <form action="/requests/get/<?=$value['id']?>" method="post">
                            <select class="form-control form-control-sm mt-1" name="id_operator" id="exampleFormControlSelect1">
                                <?foreach ($operators as $value):?>
                                    <option value="<?=$value['id']?>"><?=$value['last_name']?></option>
                                <?endforeach;?>
                            </select>
                            <button type="submit" class="btn btn-primary btn-sm mt-1">Назначить</button>
                        </form>
                    <? else:?>
